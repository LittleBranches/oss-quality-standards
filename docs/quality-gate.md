...
## Validate in a real consumer before done
...
This step is manual and sits outside `scripts/quality-gate.js` — it requires a real consumer app and human judgment about whether the result is correct, the same way visual verification does. Treat it as part of "done," not an optional nice-to-have: a change that only passes its own package's automated gate but has never been exercised inside a real consumer has not actually been validated end-to-end.
